...
def download_embedding_parquet_files_from_storage(embedding_full_path, embedding_folder_name, num_parquet_files = 1000):
    ...
    storage_account_name = 'debrajdnnhackstore'
    storage_account_key ='x2BjHZkKVswm/hAueUd9bX9WUkjsxRVWvCJ2n2D4OE9n5P+6uRrcdtJPabwVtPid4TZtCvMM6mjmO6zr1FEMZA=='
    storage_container_name ='debrajdnnhackstore' 
...
def main():
    ...
    #embedding_folder_name = "w2vmodel_entiredataset_vs_{}_ws_{}_mc_{}_with_gzip_array".\
    #    format(embed_vector_size, window_size, min_count)
    ...
    embedding_pickle_file = os.path.join(home_dir, "models", "Word2Vec_Model.p")
    ...
    model_file_path = os.path.join(home_dir,'models','old_lstm_{}_model_units_{}_lyrs_{}_epchs_{}_vs_{}_ws_{}_mc_{}.h5'.\
    ...
    with K.get_session() as sess:        
        ...
        with graphr.as_default():                        
            ...
            if mode == 'train':
                ...
                entityExtractor.train (train_file_local_path, network_type = network_type, \
...
############################################################################
# Generate the output of the model in the correct format for evaluation
############################################################################
