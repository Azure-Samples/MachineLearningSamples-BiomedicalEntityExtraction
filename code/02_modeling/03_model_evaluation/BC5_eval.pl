...
@objs=("Disease", "Chemical", "Symptom", "Drug");
$alls="[-ALL-]";
...
while ($rline = <RFILE>) {
	...
	{
		die "insufficient answers.\n"
	}
    $linenum++;

    if ($rline eq "") {
	if ($aline ne "") {die "sentence alignment error at line $linenum $aline.\n"}
